...
public class ProductController {
	...
	public String showProductPageWithLineOfProduct(@PathVariable("lineOfProduct") String lineOfProduct,@RequestParam(name = "sortByPrice",  required = false) String sortBy, Model model) {
		if(sortBy != null) {
			if(sortBy.equalsIgnoreCase("ASC")) {
				model.addAttribute("product",productService.findByLineOfProduct(lineOfProduct, Sort.by(Sort.Direction.ASC, "price")));
			} else {
				model.addAttribute("product",productService.findByLineOfProduct(lineOfProduct, Sort.by(Sort.Direction.DESC, "price")));
			}
		}
